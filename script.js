...
function draw() {
  ...
  // asks the player to jump when they press the up arrow
...
function handleObstacles(player) {
  ...
  if (rand < currentChance) {
    obstacles[obstacles.length] = new Rectangle(size, size, playerY, speed, 650);
    speed += 0.1;
    timeBetweenSpawn = 0;
  } else if (rand < currentChance * 2) {
    obstacles[obstacles.length] = new Triangle(size, size, playerY + size, speed, 650);
    speed += 0.1;
    timeBetweenSpawn = 0;
  } else {
...
function die() {
  ...
  //on death, game freezes, restart button appears
  //dead = true;
  clear();
 ...
 // restartButton.position((screenWidth - 200) / 2, 320);
  restartButton.style("font-family", "Bodoni");
